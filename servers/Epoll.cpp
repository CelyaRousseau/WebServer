...
int Epoll::make_socket_non_blocking (int sfd)
{
  int flags, s;
...
int Epoll::create_and_bind (char *port)
{
...
void Epoll::run()
{
  ...
  strs << port;
  char * tmp;
  tmp  = (char*)strs.str().c_str();
  sfd = create_and_bind (tmp);
  if (sfd == -1)
